...
class DocumentationPanel(wx.Panel):
    """Документация (встраиваемая панель)"""

    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.Size(650, 600), style=wx.TAB_TRAVERSAL, name=wx.EmptyString):
        wx.Panel.__init__(self, parent, id=id, pos=pos, size=size, style=style, name=name)

        self.SetBackgroundColour(wx.SystemSettings.GetColour(wx.SYS_COLOUR_BTNFACE))
        self.SetMinSize(wx.Size(650, -1))

        self.parent_dialog = self.GetParent()  # Родитель окна

        sizer_data = wx.BoxSizer(wx.VERTICAL)
        sizer_data.SetMinSize(wx.Size(650, 600))
